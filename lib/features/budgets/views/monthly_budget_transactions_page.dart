...
class MonthlyBudgetTransactionsPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<MonthlyBudgetTransactionsPage> createState() => _MonthlyBudgetTransactionsPageState();
}

class _MonthlyBudgetTransactionsPageState extends ConsumerState<MonthlyBudgetTransactionsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final monthString = '${DateTimeConstants.fullMonths[widget.month - 1]} ${widget.year}';
      ...
      body: transactionsAsync.when(
        ...
        data: (transactions) {
          ...
          final allMonthExpenses = transactions.where((data) {
            ...
            return tx.type == 'Expense' && tx.date.month == widget.month && tx.date.year == widget.year;
          }).toList();

          // 2. ISOLATE OUT OF BUCKET TOTAL
          double outOfBucketTotal = 0.0;
          for (var data in allMonthExpenses) {
            if (data.transaction.bucketId == null || data.transaction.bucketId == -1) {
          ...
          final displayExpenses = _showOutOfBucket 
              ? allMonthExpenses 
              : allMonthExpenses.where((data) => data.transaction.bucketId != null && data.transaction.bucketId != -1).toList();

          displayExpenses.sort((a, b) => b.transaction.date.compareTo(a.transaction.date));

          // 4. MATHEMATICAL PROJECTION ENGINE
          final now = DateTime.now();
          final isCurrentMonth = now.month == widget.month && now.year == widget.year;
          final isPastMonth = widget.year < now.year || (widget.year == now.year && widget.month < now.month);
          ...
          return CustomScrollView(
            ...
            slivers: [
                        ...
                        child: Row(
                          ...
                          children: [
                            Row(
                              children: [
                                Container(
                                  ...
                                  child: Icon(Icons.blur_circular_rounded, color: theme.colorScheme.error, size: 18),
                                ),
                                const SizedBox(width: 12),
                                Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    const Text('Out of Bucket', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13)),
                                    Text('₹${outOfBucketTotal.toStringAsFixed(0)} Total', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 11, color: theme.colorScheme.error)),
                                  ],
                                ),
                              ],
                            ),
                            Switch(
                              ...
                              onChanged: (val) => setState(() => _showOutOfBucket = val),
                            )
                          ],
                        ),
                      ),
              ...
              if (displayExpenses.isEmpty)
                SliverFillRemaining(
                  hasScrollBody: false,
                  child: Center(child: Text('No transactions logged yet.', style: TextStyle(color: theme.colorScheme.onSurfaceVariant, fontWeight: FontWeight.bold))),
                )
              else
                SliverPadding(
