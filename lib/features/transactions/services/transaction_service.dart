...
class TransactionService {      
  ...
  Future<void> logTransaction({          
    ...
    await _db.transaction(() async {              
      ...
      // --- LOAN FEE BYPASS ---
      // Ensures manual/split logs of Interest/Tax never alter the principal loan balance
      bool isLoanFee = subCategory == 'Loan Interest' || subCategory == 'Tax on Interest';
      ...
      await _db.into(_db.transactions).insert(TransactionsCompanion.insert(                   
        ...
        bucketId: Value(bucketId), bucketName: Value(bucketName),                   
  ...
  Future<void> updateTransaction({          
    ...
    await _db.transaction(() async {              
      ...
      // Reverse old transaction logically
      bool isOldLoanFee = oldTx.subCategory == 'Loan Interest' || oldTx.subCategory == 'Tax on Interest';
      if (oldTx.type == 'Expense' && !isOldLoanFee) revSourceBalance += oldTx.amount;              
      ...
      // Apply new transaction logically
      bool isNewLoanFee = subCategory == 'Loan Interest' || subCategory == 'Tax on Interest';
      ...
      await _db.update(_db.transactions).replace(oldTx.copyWith(                  
        ...
        bucketName: Value(bucketName), notes: Value(notes),                  
  ...
  Future<void> deleteTransaction(String transactionId) async {          
    await _db.transaction(() async {              
      ...
      // --- LOAN FEE BYPASS ---
      bool isLoanFee = tx.subCategory == 'Loan Interest' || tx.subCategory == 'Tax on Interest';
  ...
  Future<void> splitTransaction({          
    ...
    await _db.transaction(() async {              
      ...
      await updateTransaction(                  
        ...
        locationName: origTx.locationName, latitude: origTx.latitude, longitude: origTx.longitude,       
      );                     
  ...
  Future<void> logLoanPayment({
    ...
    await _db.transaction(() async {
      final loanAcc = await (_db.select(_db.accounts)..where((a) => a.id.equals(accountId))).getSingle();
      
      // 1. Principal Payment
      if (principal > 0) {
        double newBalance = loanAcc.balance - principal;
        await _db.update(_db.accounts).replace(loanAcc.copyWith(balance: newBalance));
